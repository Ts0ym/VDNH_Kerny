...
namespace AwakeSolutions
{
    public class AwakeSerialCom : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (Time.time - lastConnectionTryTime > 3f)
                if (serialPort == null || !serialPort.IsOpen)
                    Open(PlayerPrefs.GetString("serial_port_name"));
        }
        ...
        {
            // Save port to PlayerPrefs
            lastConnectionTryTime = Time.time;
            PlayerPrefs.SetString("serial_port_name", port);
